...
const RecordItem = ({ record }: { record: Record }) => {
  ...
  const handleDeleteRecord: (recordId: string) => Promise<void> = async (recordId: string) => {
    setIsLoading(true);
  ...
  return (
    <li
      className={`flex justify-between items-center bg-white p-4 rounded-lg shadow-md mb-4 border-t border-t-gray-100 ${
        record?.amount < 7 ? "border-l-4 border-red-500" : "border-l-4 border-green-500"
      }`}
    >
      <div className="flex flex-col">
        ...
        <span className="text-lg font-bold text-gray-800">{record?.amount} hours</span>
